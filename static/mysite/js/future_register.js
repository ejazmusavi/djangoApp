...
$(document).on('click','#future_student_sign_up_btn',function(e){
 console.log("Hello !!")  
  ...
  var highest_degree_obtained =$('#highest_degree_obtained').val();
  var subject_interested =$('#subject_interested option:selected').val();
  ...
  highest_degree_obtained = highest_degree_obtained.trim()
...
if(highest_degree_obtained.length == 0)
{
  $("#highest_degree_obtained_error").text('Enter the program studied.');
        document.getElementById("highest_degree_obtained_error").style.color = "red";

    return false;
}

if(highest_degree_obtained.length > 15)
{
  $("#highest_degree_obtained_error").text('Max length of programm studied is 15.');
        document.getElementById("highest_degree_obtained_error").style.color = "red";

    return false;
}

  if(highest_degree_obtained.length > 0 && highest_degree_obtained.length <= 15)
  {
    $("#highest_degree_obtained_error").text('')
    console.log('pin code done.')
  }


if(subject_interested.length == 0)
{
  
  $("#subject_interested_error").text('Select  the Skills Interested.');
        document.getElementById("subject_interested_error").style.color = "red";
    return false;
}

if(subject_interested.length !=0 )
{
$("#subject_interested_error").text('');
}
...
$("#country_error").text('');
  ...
  $.ajax({
    url: '/future_registration/',
    type: "POST",
    data: {'first_name':first_name,'surname':surname,'date_of_birth':date_of_birth,'email':email,'address':address,'pin_code':pin_code,'grades_obtained':grades_obtained,'highest_degree_obtained':highest_degree_obtained,'subject_interested':subject_interested,'max_grades':max_grades,'password':password,'country':country,'csrfmiddlewaretoken':token },
    dataType: 'json',
    cache: false,
